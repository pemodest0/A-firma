...
from scripts.agro.run_agro_br_monthly_pipeline import validate_latest_artifacts
...
def test_validate_latest_artifacts_ok(tmp_path):
    ...
    (latest / "hierarchical_state_latest_agro_br.json").write_text(
        json.dumps(
            {
                "date": "2026-02-28",
                "global_score": 0.1,
    ...
    (latest / "rankings_latest_agro_br.json").write_text(
        json.dumps(
            {
                "date": "2026-02-28",
    ...
    (latest / "historical_structure_summary_agro_br.json").write_text(
        json.dumps(
            {
...
def test_validate_latest_artifacts_missing_key(tmp_path):
    ...
    assert checks["hierarchical_state_latest_agro_br.json"]["ok"] is False
