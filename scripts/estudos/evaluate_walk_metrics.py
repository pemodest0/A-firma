...
BENCHMARK_DIR = REPO_ROOT / "dados" / "benchmarks" / "metrics_output"
...
def main() -> None:
    ...
    np.save(BENCHMARK_DIR / "diffusion.npy", diffusion)
    np.save(BENCHMARK_DIR / "autocorr.npy", autocorr)
    np.save(BENCHMARK_DIR / "prob_gap.npy", probs_gap)
    print(f"Metrics saved to {BENCHMARK_DIR}")
