...
from execution.shadow_gods import build_shadow_gods_summary, load_shadow_gods_profile  # noqa: E402
...
def _current_as_of_date(operation: dict[str, Any], vigilance: dict[str, Any]) -> str:
    return str(
        operation.get("inputs_as_of_date")
        or operation.get("as_of_date")
        or vigilance.get("as_of_date")
        or ""
    ).strip()


def _can_reuse(previous: dict[str, Any], *, as_of_date: str) -> bool:
    if str(previous.get("status") or "").strip().lower() != "ok":
        return False
    return str(previous.get("as_of_date") or "").strip() == str(as_of_date).strip()


def main() -> None:
    ap = argparse.ArgumentParser(description="Agente diário dos quatro shadow gods com três blocos de capital.")
    ...
    ap.add_argument("--outdir-root", default="results/ops/agents/daily_shadow_gods")
    ...
    as_of_date = _current_as_of_date(operation, vigilance)
    ...
    if not operation:
        summary = {
            ...
            "notes": ["O agente diário de shadow gods depende do latest_summary do daily_operation."],
        }
    elif not as_of_date:
        summary = {
            ...
            "cycle_run_id": cycle_run_id,
            "agent_run_id": agent_run_id,
            "notes": ["Não foi possível resolver a data-base da simulação diária."],
        }
    elif _can_reuse(previous, as_of_date=as_of_date):
        ...
        built = build_shadow_gods_summary(
            repo_root=ROOT,
            profile=profile,
            operation=operation,
            vigilance=vigilance,
        ...
        summary["operation_summary_path"] = str((ROOT / args.operation_summary).resolve())
        summary["vigilance_summary_path"] = str((ROOT / args.vigilance_summary).resolve())
    ...
    _write_json(outroot / "latest_shadow_gods.json", summary)
