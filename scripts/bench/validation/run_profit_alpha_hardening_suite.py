...
def _build_promoted_attack_confidence_score(context: dict[str, Any], attack_returns: pd.DataFrame) -> pd.Series:
    ...
    crypto_fast = _tail_return(crypto_signal_ret, 21)
    equity_fast = _tail_return(equity_signal_ret, 21)
    crypto_slow = _tail_return(crypto_signal_ret, 63)
...
def _build_alpha_meta_allocation_bundle(
    *,
    ...
    crypto_signal_ret = crypto_ret.shift(1).fillna(0.0)
    equity_signal_ret = equity_ret.shift(1).fillna(0.0)
    crypto_entry = _tail_return(crypto_signal_ret, int(entry_lookback))
    equity_entry = _tail_return(equity_signal_ret, int(entry_lookback))
    crypto_exit = _tail_return(crypto_signal_ret, int(exit_lookback))
    equity_exit = _tail_return(equity_signal_ret, int(exit_lookback))
