...
def main() -> None:
    ...
    outdir = Path("analysis/embed_output")
    ...
    for embedding_fn in (pca_embedding, tsne_embedding):
        ...
        ax.set_ylabel("y")
        fig.tight_layout()
        fig.savefig(outdir / f"{emb.method.lower()}_step{args.step}.png", dpi=200)
        plt.close(fig)
    ...
    ax.set_aspect("equal")
    fig.savefig(outdir / "hyperbolic.png", dpi=200)
    plt.close(fig)

    print(f"Embeddings saved under {outdir}")
